...
export function SettingsView() {
  ...
  const [autocorrectIgnoreCaps, setAutocorrectIgnoreCaps] = useState(true);
  ...
  useEffect(() => {
    ...
    const savedAutocorrectIgnoreCaps = localStorage.getItem(AUTOCORRECT_IGNORE_CAPS_STORAGE_KEY);
    setAutocorrectIgnoreCaps(savedAutocorrectIgnoreCaps !== "false");
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem(AUTOCORRECT_IGNORE_CAPS_STORAGE_KEY, String(autocorrectIgnoreCaps));
  }, [autocorrectIgnoreCaps]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-3xl mx-auto px-10 py-8 space-y-8">
          ...
          <section className="space-y-3">
            <h2 className="text-lg text-[#e3e3e3] font-medium">Writing</h2>
            <p className="text-sm text-[#9b9b9b]">Control editor autocorrect behaviour.</p>
            <div className="rounded border border-[#2f2f2f] bg-[#1e1e1e] p-4 space-y-3">
              ...
              <label className="flex items-center gap-2 text-sm text-[#d1d1d1] cursor-pointer">
                <input
                  type="checkbox"
                  checked={autocorrectIgnoreCaps}
                  onChange={() => setAutocorrectIgnoreCaps((prev) => !prev)}
                  className="h-4 w-4 accent-[#7eb8f7]"
                />
                <span>Ignore words starting with a capital letter</span>
              </label>
            </div>
          </section>
        </div>
